...
### Fixed
- **Boot failure with `newrelic_rpm`**: `Trackplot::ChartHelper` is now explicitly required in the engine initializer before the `on_load(:action_view)` hook, preventing an `uninitialized constant` error when another gem loads Action View before Zeitwerk is ready.
